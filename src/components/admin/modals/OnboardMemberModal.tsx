  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (res.data.status === "success" || res.data.status === true as unknown as string) {
        const successMsg = recordPayment
          ? "Member added with payment recorded — ready for President to sign the certificate"
        ...
        <div className="p-6 space-y-4">
          ...
          {step === 3 && (<>
            ...
            <div className="rounded-lg bg-gray-50 dark:bg-gray-700/30 p-3 text-xs text-gray-500 dark:text-gray-400 space-y-1">
              ...
              {recordPayment ? (
                <p>→ Welcome email sent · payment recorded · status <span className="font-medium">Approved</span> · <span className="font-medium">President signs the certificate</span></p>
              ) : (
