...
function getStatusColorFromStatus(status: string) {
  const s = status?.toLowerCase();
  if (s === "certgenerated" || s === "certificate generated") return "bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-400";
      ...
      <div className="bg-gradient-to-r from-[#00B5A5] to-[#00D4C7] rounded-xl p-6 text-white shadow-lg">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-[#E6FFFD] text-sm">
              {!application
                ? "Submit your application to get started"
                : appStatus.toLowerCase() === "pending"
                ? "Your application is under review"
                : appStatus.toLowerCase() === "waiting for payment"
                ? "Complete your payment to activate membership"
                : appStatus.toLowerCase() === "approved"
                ? "Payment received — awaiting certificate"
                : appStatus.toLowerCase() === "certgenerated" || appStatus.toLowerCase() === "certificate generated"
                ? "Track your membership, certificates, and events"
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {[
          ...
          <button
            ...
            className="flex flex-col items-center gap-2 p-4 rounded-xl bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700/50 hover:shadow-sm transition-all"
          >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <button onClick={() => router.push("/my-application")} className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-5 text-left hover:shadow-md transition-shadow">
          <div className="flex items-center justify-between mb-3">
            ...
            <span className={`px-2.5 py-0.5 rounded-full text-xs font-medium ${getStatusColor(appStatus)}`}>
              {getStatusLabel(appStatus)}
            </span>
          </div>
          <p className="text-sm font-medium text-gray-900 dark:text-white">Application</p>
          <p className="text-xs text-gray-500 dark:text-gray-400 mt-0.5">
            {application?.organization || "View your membership application"}
          </p>
        </button>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-5">
          ...
          <p className="text-sm font-medium text-gray-900 dark:text-white">Next Renewal</p>
          <p className="text-xs text-gray-500 dark:text-gray-400 mt-0.5">
            {next_renewal_date ? formatDate(next_renewal_date) : "No renewal scheduled"}
          </p>
        </div>
      </div>
